...
function sessiondumpf {(Get-View SessionManager).SessionList | Select Key, UserName, FullName, LoginTime, LastActiveTime, IpAddress, CallCount | Sort-Object UserName, IpAdress, LoginTime | epcsv -NoType "session-list_$($global:defaultVIServer.Name)_$(Get-Date -F FileDateTime).csv"}
